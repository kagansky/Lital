...
public class SnakeGame extends JFrame {
	...
	private void updateGame() {
		...
		if(collision == TileType.Banna) {
			fruitsEaten++;
			score += nextFruitScore;
			spawnFruit();
		}
		if(collision == TileType.Question_Mark) {
			fruitsEaten++;
			score += nextFruitScore;
			spawnFruit();
		}
		if(collision == TileType.Pear) {
			fruitsEaten++;
			score += nextFruitScore;
			spawnFruit();
		}
	...
	private TileType updateSnake() {
		...
		if(old != TileType.Banna && snake.size() > MIN_SNAKE_LENGTH) {
			Point tail = snake.removeLast();
			board.setTile(tail, null);
			old = board.getTile(head.x, head.y);
		}
		if(old != TileType.Question_Mark && snake.size() > MIN_SNAKE_LENGTH) {
			Point tail = snake.removeLast();
			board.setTile(tail, null);
			old = board.getTile(head.x, head.y);
		}
		if(old != TileType.Pear && snake.size() > MIN_SNAKE_LENGTH) {
			Point tail = snake.removeLast();
			board.setTile(tail, null);
			old = board.getTile(head.x, head.y);
		}
	...
	private void spawnFruit() {
		...
		for(int x = 0; x < BoardPanel.COL_COUNT; x++) {
			for(int y = 0; y < BoardPanel.ROW_COUNT; y++) {
				...
				if(type == null || type == TileType.Banna) {
					if(++freeFound == index) {
						board.setTile(x, y, TileType.Banna);
						break;
					}
					if(type == null || type == TileType.Question_Mark) {
						if(++freeFound == index) {
							board.setTile(x, y, TileType.Question_Mark);
							break;
						}
				if(type == null || type == TileType.Pear) {
					if(++freeFound == index) {
						board.setTile(x, y, TileType.Pear);
							break;
						}	
